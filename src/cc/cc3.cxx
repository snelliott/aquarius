...
template <typename U>
CC3<U>::CC3(const string& name, Config& config)
...
template <typename U>
bool CC3<U>::run(task::TaskDAG& dag, const Arena& arena)
{
    ...
    this->puttmp("QABEJ", new SpinorbitalTensor<U>(" Q(ab,ej)", H.getABCI()));
    this->puttmp("QAMIJ", new SpinorbitalTensor<U>( "Q(am,ij)", H.getAIJK()));
    this->puttmp("WABNFIJ", new SpinorbitalTensor<U>("W(abn,fij)", arena,
                                               H.getABIJ().getGroup(),
                                               {vrt, occ}, {2,1},
                                               {1,2}));
...
template <typename U>
void CC3<U>::iterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
    ...
    WABEJ["abej"] -=     WAMEI["amej"]*T(1)[    "bm"];//W
    ...
    T3["abcijk"]    =      WABEJ["bcek"]*T(2)[  "aeij"];
    T3["abcijk"]   -=      WAMIJ["bmjk"]*T(2)[  "acim"];
...
template <typename U>
void CC3<U>::subiterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CC3);
REGISTER_TASK(aquarius::cc::CC3<double>,"cc3",spec);
