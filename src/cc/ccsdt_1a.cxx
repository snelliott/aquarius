...
template <typename U>
CCSDT_1a<U>::CCSDT_1a(const string& name, Config& config)
: Subiterative<U>(name, config), diis(config.get("diis"))
{
    ...
    this->addProduct(Product("ccsdt_1a.T",  "T", reqs));
    this->addProduct(Product("ccsdt_1a.T3", "T3", reqs));
    this->addProduct(Product("ccsdt_1a.Hbar", "Hbar", reqs));
}

template <typename U>
bool CCSDT_1a<U>::run(task::TaskDAG& dag, const Arena& arena)
{
...
template <typename U>
void CCSDT_1a<U>::iterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
...
template <typename U>
void CCSDT_1a<U>::subiterate(const Arena& arena)
{
    const auto& H = this->template get<TwoElectronOperator<U>>("H");
...
INSTANTIATE_SPECIALIZATIONS(aquarius::cc::CCSDT_1a);
REGISTER_TASK(aquarius::cc::CCSDT_1a<double>,"ccsdt-1a",spec);
